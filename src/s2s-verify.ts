...
export function verifyS2sHeader(
  ...
  // Non-null assertion: this repo's tsconfig sets noUncheckedIndexedAccess,
  // which types capture-group access as `string | undefined`. match[2] is a
  // mandatory (non-optional) group in HEADER_VALUE_RE, so it is always
  // defined once `match` itself is non-null (checked above) — no behavior
  // change from the reference implementation, compile-time only.
